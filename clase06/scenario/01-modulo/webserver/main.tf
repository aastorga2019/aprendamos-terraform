resource "google_compute_network" "vpc_network" {
  name                    = "vpc-network-${var.ambiente}"
  auto_create_subnetworks = false
}

resource "google_compute_subnetwork" "sub_network" {
  name          = "sub-network-${var.ambiente}"
...
resource "google_compute_firewall" "fw_access_80_22" {
  ...
  target_tags = ["webserver${var.ambiente}"]
}

resource "google_compute_instance" "webserver" {
  name         = "webserver-${var.ambiente}"
  machine_type = "n1-standard-1"
  zone         = "us-central1-a"

  tags = ["webserver${var.ambiente}", "mywebserver"]
